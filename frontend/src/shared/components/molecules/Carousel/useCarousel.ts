...
export function useCarousel(props: CarouselProps) {
    ...
    const updateVisuals = useCallback(() => {
        ...
        const layouts = calculateVisuals(totalItems, progress, !isInfinite, isMobile);
        ...
        const currentLogicalIndex = ((currentIndex % totalItems) + totalItems) % totalItems;
        ...
        if (activeIndexRef.current !== originalIndex) {
            ...
            if (!dragState.current.isAnimating && !dragState.current.isDown && isAuto) {
    ...
    const playAuto = useCallback(() => {
        ...
        if (activeBar) {
            progressTweenRef.current = gsap.fromTo(activeBar,
                { scaleX: 0 },
    ...
    const onPointerDown = (e: PointerEvent<HTMLDivElement>) => {
        ...
        const onPointerUp = (upEvent: globalThis.PointerEvent) => {
            ...
            if (isMobile) {
                const startIndex = Math.round(dragState.current.scrollLeft / scroller.clientWidth);
                if (isHorizontalSwipe) {

    ...
    <aside className="hidden md:block relative w-32 h-full drop-shadow-2xl">
      ...
      <div className="absolute inset-y-0 left-0 right-6 bg-[var(--wb-back-bg)] rounded-tr-[16px] rounded-br-[16px] z-10 flex flex-col justify-end items-end pb-6 pr-3">
        <div className="flex flex-col space-y-4">
          ...
          >
            <InfoIcon
              className={`w-6 h-6 transition-transform duration-300 ${activeSecondary === "info" ? "text-[var(--wb-icon-active)]" : "group-hover:scale-110 group-hover:text-[var(--wb-icon-hover)]"}`}
            />
          </button>
          ...
          <button
            onClick={(e) => {
              if (onSecondaryClick) {
                ...
                setActiveSecondary(
                  activeSecondary === "settings" ? null : "settings",
                );
              }
            }}
            title="Settings"
          ...
          >
            <SettingsIcon
              className={`w-6 h-6 transition-transform duration-300 ${activeSecondary === "settings" ? "rotate-90 text-[var(--wb-icon-active)]" : "group-hover:rotate-90 group-hover:text-[var(--wb-icon-hover)]"}`}
            />
          </button>
        </div>
      </div>
      ...
      <div className="absolute inset-0 z-20 flex flex-col pointer-events-none drop-shadow-[4px_0_8px_rgba(0,0,0,0.6)]">
        <div className="w-32 bg-[var(--wb-front-bg)] rounded-tr-[16px] flex flex-col items-center pt-8 pb-4 pointer-events-auto">
          ...
          <nav
            ref={navRef}
            className="flex flex-col items-center w-full space-y-4 relative"
          >
            {/* Active Indicator */}
            <div
              ref={indicatorRef}
            ...
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  <item.icon
                    className={`w-8 h-8 mb-1 transition-transform duration-300 ${isActive ? "text-[var(--wb-icon-active)]" : "group-hover:scale-110 group-hover:text-[var(--wb-icon-hover)]"}`}
                  />
        ...
        <div className="w-32 h-20 pointer-events-auto relative -mt-[1px]">
          ...
          >
            <path
              d="M 80 0 C 80 32 20 32 20 64 L 0 64 L 0 0 Z"
              fill="var(--wb-front-bg)"
            />
          </svg>
        </div>

 ...
 */
export const FeaturedMods: React.FC<FeaturedModsProps> = React.memo(({
  onCardClick,
  searchQuery = "",
  engineIds = ["all"],
}) => {
  ...
  if (displayMods.length === 0) {
    ...
    return (
      <div className="mb-4 w-full">
        ...
        <div className="w-full aspect-[2/1] sm:aspect-[21/9] lg:aspect-[21/8] xl:aspect-[3/1] bg-[var(--wb-surface-container)] rounded-[32px] flex flex-col items-center justify-center border border-white/5 shadow-none mt-4 p-6 text-center">
          <AlertTriangle className="w-12 h-12 text-[var(--wb-primary)] mb-4 opacity-80" />
          <h3 className="text-xl font-bold text-[var(--wb-on-surface)] mb-2">
              ...
              <div className="flex items-center justify-center gap-4 mt-4 px-4 w-full pointer-events-none z-10">
                {!searchQuery && (
                  <button
                    ...
                    className="p-2 rounded-full bg-[var(--wb-carousel-arrow-bg)] hover:bg-[var(--wb-carousel-arrow-hover)] text-[var(--wb-carousel-arrow-text)] border border-[var(--wb-carousel-arrow-border)] transition-all duration-200 shrink-0 pointer-events-auto cursor-pointer shadow-md backdrop-blur-md hover:scale-105 active:scale-95"
                    aria-label="Anterior mod destacado"
                  >
                    <ChevronLeft size={20} />
                  </button>
                )}
                      ...
                      <div
                        ...
                        className={`relative h-2 rounded-full overflow-hidden transition-all duration-300 cursor-pointer ${
                          api.activeIndex === mod.absoluteIndex
                            ? "w-8 bg-[var(--wb-carousel-dot-active)]"
                            : "w-2 bg-[var(--wb-carousel-dot-inactive)] hover:bg-[var(--wb-carousel-dot-hover)]"
                        }`}
                ...
                {!searchQuery && (
                  <button
                    ...
                    className="p-2 rounded-full bg-[var(--wb-carousel-arrow-bg)] hover:bg-[var(--wb-carousel-arrow-hover)] text-[var(--wb-carousel-arrow-text)] border border-[var(--wb-carousel-arrow-border)] transition-all duration-200 shrink-0 pointer-events-auto cursor-pointer shadow-md backdrop-blur-md hover:scale-105 active:scale-95"
                    aria-label="Siguiente mod destacado"
                  >
            ...
            >
              <div className="m3-card w-full h-full bg-[var(--wb-surface-container)] rounded-[32px] relative shadow-none border border-white/5">
                ...
                {!searchQuery && item.__featuredLabel && (
                  <div className="m3-card-badge absolute top-6 left-6 sm:top-8 sm:left-8 z-10 bg-black/40 backdrop-blur-md px-4 py-2 rounded-full shadow-none border border-white/10 pointer-events-auto flex items-center gap-2">
                        ...
                        {item.engineId &&
                          Object.values(ENGINE_CATEGORIES).find(
                            ...
                            >
                              ...
                              <div className="flex px-4 py-2 bg-[var(--wb-surface-container-highest)] text-[var(--wb-on-surface)] text-sm font-bold rounded whitespace-nowrap shadow-none border border-white/10">
                                {
                                  Object.values(ENGINE_CATEGORIES).find(
                                    (c) => c.id === item.engineId,
                                  )?.name

    ...
    >
      {selectedCard && (
        <div className="flex flex-col gap-4 text-[var(--wb-on-surface)]">
          ...
          {selectedCard.htmlBody ? (
            <div
              className="text-[var(--wb-on-surface-variant)] prose prose-invert max-w-none"
              dangerouslySetInnerHTML={{ __html: selectedCard.htmlBody }}
            />
          ) : (
            <p className="text-[var(--wb-on-surface-variant)]">

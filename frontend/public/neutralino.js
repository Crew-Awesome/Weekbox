  ...
  function d() {
    ...
    ((a = new WebSocket(`ws://${o}:${window.NL_PORT}?connectToken=${e}`)),
          ...
          ? (t.data?.error
              ? (s[t.id].reject(t.data.error),
                "NE_RT_INVTOKN" == t.data.error.code &&
                  (a.close(),
                  console.error(
                    "NE_RT_INVTOKN: Neutralinojs application cannot execute native methods since NL_TOKEN is invalid.",
                  )))
              : t.data?.success &&
                s[t.id].resolve(
      ...
      a.addEventListener("error", async (e) => {
        console.error(
          "NE_CL_IVCTOKN: Neutralinojs application cannot connect with the framework core using NL_TOKEN.",
        );
      }));
  }
  function l(e, t) {

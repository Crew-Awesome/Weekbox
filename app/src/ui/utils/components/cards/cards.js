...
export function initCards(parentContainer = document) {
  const cards = parentContainer.querySelectorAll(
    '.md3-component-card:not([data-init="true"])',
  );
  ...
  cards.forEach((card) => {
    ...
    if (scheme) {
      card.style.setProperty(
        "--card-bg",
        `var(--m3-component-${scheme}-container)`,
      );
      card.style.setProperty(
        "--card-color",
        `var(--m3-component-on-${scheme}-container)`,
      );
    }
    ...
    if (customBg)
      card.style.setProperty("--card-bg", parseColor(customBg, "transparent"));
    if (customColor)
      card.style.setProperty(
        "--card-color",
        parseColor(customColor, "inherit"),
      );
